...
const fetchSortedBooks = async () => {
  ...
  try {
    const response = await fetch(`http://localhost:5000/api/v1/booksFilter?${queryParams.toString()}`);
...
fetchSortedBooks();
const createBook = document.getElementById('create-book-div') as HTMLDivElement;
const btnCreateBook = document.getElementById('btn-create-book') as HTMLButtonElement;
...
document.querySelector('#postBooks')?.addEventListener('submit', async(event)=>{
  ...
  try {
    ...
    if (response.ok) {
        ...
        const form = document.getElementById("postBooks") as HTMLFormElement;
        form.reset();   
       } else {
