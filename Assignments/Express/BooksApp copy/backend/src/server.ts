...
app.get('/api/booksFilter', (req:Request, res:Response) => {
    try {
        ...
        if(sortBy) {
            if (sortBy === 'Pages' && order ==='Ascending') {
                filteredBooks = filteredBooks.sort((book1, book2)=>book1.pages - book2.pages)
            }

            if (sortBy === 'Pages' && order ==='Descending') {
                filteredBooks = filteredBooks.sort((book1, book2)=>book2.pages - book1.pages)
            }

            if (sortBy === 'Years' && order ==='Ascending') {
                filteredBooks = filteredBooks.sort((book1, book2)=>book1.year - book2.year)
            }

            if (sortBy === 'Years' && order ==='Descending') {
                filteredBooks = filteredBooks.sort((book1, book2)=>book2.year - book1.year)
            }
        }
